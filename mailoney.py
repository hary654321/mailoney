...
parser.add_argument(
    ...
    action="store",
    metavar="mailserver",
    default=os.environ.get("MAILSERVER_NAME", None),
    help="A Name that'll show up as the mail server name",
)
parser.add_argument(
    "-t",
    action="store",
    choices=["open_relay", "postfix_creds", "schizo_open_relay"],
    required=True,
    help="honeypot type",
)
parser.add_argument(
    ...
    action="store",
    metavar="<logpath>",
    default=os.environ.get("LOGPATH"),
    help="path for file logging",
)
parser.add_argument(
    "-hpfserver",
    action="store",
    metavar="<hpfeeds-server>",
    default=os.environ.get("HPFEEDS_SERVER", None),
    help="HPFeeds Server",
)
parser.add_argument(
    "-hpfport",
    action="store",
    metavar="<hpfeeds-port>",
    default=os.environ.get("HPFEEDS_PORT", None),
    help="HPFeeds Port",
)
parser.add_argument(
    "-hpfident",
    action="store",
    metavar="<hpfeeds-ident>",
    default=os.environ.get("HPFEEDS_IDENT", None),
    help="HPFeeds Username",
)
parser.add_argument(
    "-hpfsecret",
    action="store",
    metavar="<hpfeeds-secret>",
    default=os.environ.get("HPFEEDS_SECRET", None),
    help="HPFeeds Secret",
)
parser.add_argument(
    "-hpfchannelprefix",
    action="store",
    metavar="<hpfeeds-channel-prefix>",
    default=os.environ.get("HPFEEDS_CHANNELPREFIX", None),
    help="HPFeeds Channel Prefix",
)
...
def connect_hpfeeds():
    ...
    if (
        ...
        try:
            hpc = hpfeeds.new(
                hpfeeds_server, int(hpfeeds_port), hpfeeds_ident, hpfeeds_secret
            )
